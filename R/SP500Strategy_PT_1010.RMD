...
if (str_detect(.Platform$OS.type, "win")) {
  ArchiveFile = "C:\\Users\\Chris.Woolery\\OneDrive - Republic Airways\\Documents\\RepoArchive\\SavvyDemoProjects\\R\\SP500Strategy_PT1010.RData"
} else {
...
if (str_detect(.Platform$OS.type, "win")) {
  ...
  Plot1 = DisplayDF %>%
    ggplot(aes(WeekBeg, SP500)) +
      ...
      geom_line(aes(WeekBeg, FedFundsAlt, color = "Fed Funds Rate (Not to scale)"), size = 1) +
  ...
  Plot1 = DisplayDF %>%
    ggplot(aes(WeekBeg, SP500)) +
      ...
      geom_line(aes(WeekBeg, FedFundsAlt, color = "Fed Funds Rate (Not to scale)"), linewidth = 1) +
      geom_point(aes(WeekBeg, HighValue, color = "Peak to Date"), size = 2) +
      geom_point(aes(WeekBeg, LowValue, color = "Trough Since Last Peak"), size = 2) +
      scale_color_manual(name = "", values = c("Weekly Closing Value" = "black",
                                               "Fed Funds Rate (Not to scale)" = "dark blue",
